...
- leancloud项目会通过jenkins识别github tag行为。daocloud项目能自主识别github tag行为。两者识别后的行为都是发布app到指定的相应环境中。
...
----
1.rollbar改为bugsnag
2.jenkins改为 开发环境 和 生产环境 两个job，测试和发布各个环境代码在一个job
3.加一下python leanengine项目的部署。其实跟nodejs一样
